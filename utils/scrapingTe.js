    ...
    try{
        ...
        //·················CONTENEDOR DE ADS GENERAL Y LINKS·····················//
            ...
            // Extraemos en un array los links de las ofertas de trabajo
                ...
                const allLinks = await page.evaluate(() => {
                    const cards = document.querySelectorAll('div[class="p-2 border-bottom bg-white"]')            
                    const linksArr = [];
                    
                    cards.forEach(element=>{             
                        linksArr.push(element.getAttribute('onclick').slice(14).replace(/[ '"]+/g, ''))
                    })      
        ...
        let allAdsTec = []
            ...
            const allAds = await page.evaluate(() => {
                return {
                    adTitle: document.querySelector('h1[itemprop="title"]').innerText,
                    adCompany: document.querySelector('a[class="text-primary link-muted fs--18"]').innerText,
                    adLocation: `${document.querySelector('div[class="ml-0 mt-2"] a').innerText} - ${document.querySelector('div[class="ml-0 mt-2"] span').innerText}`,
                    adDescription: document.querySelector('p[class="fs--16 text-gray-800"]').innerText,
                    adImage: document.querySelector('img').src,
                    adDate: document.querySelector('span[class="ml-4"]').innerText,
                    adLink: `${document.querySelector('form[name="enviar"]').getAttribute('action')}?refer=${document.querySelector('form[name="enviar"] input').getAttribute('value')}&TK=`
                };
            });
